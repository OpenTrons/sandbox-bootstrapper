...
import sys
import datetime
import bootstrapper_publisher

driver = None

class Subscriber():
    def __init__(self, harness=None, publisher=None):
        print(datetime.datetime.now(),' - bootstrapper_subscriber.__init__:')
        print('\targs:',locals())
    ...
    def set_harness(self, harness):
        print(datetime.datetime.now(),' - bootstrapper_subscriber.set_harness:')
        print('\targs:',locals())
        self.harness = harness


    def dispatch_message(self, message):
        print(datetime.datetime.now(),' - bootstrapper_subscriber.dispatch_message:')
        print('\targs:',locals())
        try:
